...
bot.start(async (ctx) => {
    ...
    await ctx.replyWithMarkdown('I am a telegram bot that knows everything about movies and TV series.\n\n' +
...
bot.on('text', async (ctx) => {
    console.log([ctx.from.first_name, ctx.from.last_name].join(' '), ctx.message.text);
    const chunk = ` You: ${ctx.message.text}\n`;
    ctx.session.prompt || (ctx.session.prompt = prompt);
    ctx.session.prompt = ctx.session.prompt.substring(Math.max(0, ctx.session.prompt.length + chunk.length - 2000)) + chunk;
